...
repo_dir = plotting_dir.parent
expected = repo_dir / "expected_abundances/control_only_no_nfw_agg.tsv"
platform_dir = repo_dir / "platforms"

def get_sample_file(platform, background, file_type, mixture, suffix):
    """Returns file path based on sample specifications"""
    return platform_dir / f"{platform}/{background}/{file_type}/{mixture}.{suffix}"

def get_mosdepth_summary_file(platform, background, mixture):
    """Returns path to mosdepth summary"""
    return get_sample_file(platform, background, "mosdepth", mixture, "mosdepth.summary.txt")

def get_mosdepth_regions_file(platform, background, mixture):
    """Returns path to mosdepth regions report"""
    return get_sample_file(platform, background, "mosdepth", mixture, "regions.bed.gz")

def get_mosdepth_threshold_file(platform, background, mixture):
    """Returns path to mosdepth thresholds report"""
    return get_sample_file(platform, background, "mosdepth", mixture, "thresholds.bed.gz")

def get_simple_mean_coverage(platform, background, mixture):
    """Returns mean coverage for requested sample"""
    df = pd.read_csv(
        get_mosdepth_summary_file(platform, background, mixture), sep="\t"
    )
    return df.loc[(df["chrom"] == "MN908947.3") & (df["length"] == 29903), "mean"].values[0]

def get_freyja_aggregate_file(platform, background):
    """Returns path to freyja aggregate file for given sample set"""
    return repo_dir / f"freyja/agg/{platform}-{background}.tsv"
...
mixtures2drop = ["Mixture19", "Mixture20", "Mixture23", "Mixture41", "Mixture42", "NFWC", "NFWA"]
mixtures2drop_samplesonly = ["Mixture19", "Mixture20", "Mixture23", "Mixture41", "Mixture42"]
...
### Sample info
# Details about all the sample types we have
platform_renames = {
    "illumina_ss": "Illumina (unpaired)",
    "illumina_minimap2": "Illumina (minimap2)",
    "illumina_bwa": "Illumina (bwa)",
    "cg": "CG",
    "ont": "ONT"
}
platforms = list(platform_renames.keys())
backgrounds = ["wb","nwrb","pwrb"]
mixtures = [x for x in mixture_renames.keys() if x.startswith("Mixture")]
...
def sort_by_name(cols):
    """Returns samples sorted with NFWs first then alphabetically"""
    sample_cols = []
    nfw_cols = []
    for col in cols:
...
def getHeatmap(df,field,title=None,labels=None,title_y=0.7,height=None,width=None):
    """Returns a heatmap
    
    Args:
        `df` (DataFrame): the dataframe to use
        `field` (str): the name of the column containing values of interest
        `labels` (dict): maps x and y to new names
        `title_y` (float): y position of title for moving it up and down. Defaults to 0.7.
    """
    fig_df = df[["batch","mixture",field]].pivot(index="batch",columns="mixture",values=field)
    fig_df = fig_df[sort_by_name(fig_df.columns)]
    fig = px.imshow(fig_df, title=title, labels=labels, height=height, width=width)
...
def tukey2df(batch_res, batch_details):
    """Returns df array of tukey's HSD values for the ANOVA
    Args:
        `batch_details` (dict): {batch_name -> batch_values}
        `batch_res` (TukeyHSDResult): result of running `tukey_hsd` with `batch_details`
    """
...
def get_mean(df, col, value_col, batch_col="batch"):
    """Returns mean of the values in `value_col` for samples called `col` 
    in the `batch_col` field of the provided `df`"""
    return df[df[batch_col]==col][value_col].mean()

def get_std_dev(df, col, value_col, batch_col="batch"):
    """Returns standard deviation of the values in `value_col` 
    for samples called `col` in the `batch_col` field of the provided `df`"""
    return df[df[batch_col]==col][value_col].std()

def get_mean_and_dev(df, row_name, value_col, batch_col="batch"):
    """Returns mean and standard deviation of the values in `value_col` 
    for samples called `col` in the `batch_col` field of the provided `df`"""
...
def get_mean_and_dev_from_values(batch, values):
    """Returns mean and standard deviation of the `values` labeled by `batch`"""
...
def p_value_table(tukey_df, width=1200, height=550):
    """Returns a table of the tukey's HSD p-values with the specified dimensions"""
    fill_color = [['white']*len(tukey_df.columns)]
    fill_color.extend([
        ["black" if pd.isna(x) else "white" if x<=0.01 else "lightgrey" for x in tukey_df[col]] 
